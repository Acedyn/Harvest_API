...
-- ########################################
    ...
    FROM
        ...
        -- Get the array of layer from the metadata
        string_to_array(regexp_replace(job_metadata->>'layers', '[\[\]\s\.]', '', 'g'), ',', '')  as layers,
        ...
        WHERE state = 'done' AND job_metadata->>'project' != 'TEST_PIPE' AND job_metadata->>'layers' != '' AND job_metadata->>'layers' != '') AS metadata, invocation, blade

    -- Join the subquery to the invocation and the blade table
    WHERE metadata.jid = invocation.jid AND metadata.tid = invocation.tid AND blade.bladeid = invocation.bladeid
