...
import { get_authentification_tsid } from "./authentification"
...
async function createRunningJobFilter(filterName: string = "running_jobs") {
  ...
  const tsid = await get_authentification_tsid()
...
export async function getProjectUsage() {
  ...
  const tsid = await get_authentification_tsid();
  const running_jobs: any = await axios.get(`${process.env.TRACTOR_URL}/monitor?q=jobs&metadata=0&filter=${runningJobsFilter}&tsid=${tsid}`);
  
  const userJobs = Object.values(running_jobs.data.users).map((user: any) => Object.values(user));
  ...
  runningJobs.forEach((runningJob) => {
    ...
    runningJob.projects.forEach((project: string) => {
      projectUsage[project] = project in projectUsage ? taskCount + projectUsage[project] : taskCount
    })
  })
