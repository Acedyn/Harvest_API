...
export function getProjectHistory(app: Application) {
  app.get('/history/project-usage', async (req: Request<{}, {}, {}, {start?: string, end?: string, project?: string}>, res: Response) => {
    const [start, end] = getTimeRange(req)
    const projectRecords = await getProjectRecords(new Date(start), new Date(end), req.query.project);
    const groupedProjectRecords: {[time: string]: {[name: string]: number}} = {};
    projectRecords.forEach((projectRecord) => {
      ...
      groupedProjectRecords[index][projectRecord.projectName] = projectRecord.usage
    })

    const formattedProjectRecords: {[key: string]: number | Date}[] = []
    for (const [key, value] of Object.entries(groupedProjectRecords)) {
      formattedProjectRecords.push({...value, createdAt: new Date(parseInt(key))})
    }
